...
#if UNITY_EDITOR
[CustomEditor(typeof(Individual_RestDataReader))]
public class Individual_RestDataReaderEditor : Editor
{
    private Individual_RestDataReader data;

    // 1행 한글설명 / 2행 영문헤더 / 3행 타입 / 4행부터 데이터
    private const int DATA_START_ROW_INDEX = 4; // (0-based) 4행

    void OnEnable()
    {
        data = (Individual_RestDataReader)target;
    }
    ...
    void UpdateMethodOne(GstuSpreadSheet ss)
    {
        ...
        Debug.Log($"[Player_SpeciesDataReader] Loaded: {data.DataList.Count} rows");
    }
